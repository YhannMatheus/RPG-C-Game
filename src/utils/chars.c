...
void printCharacter(Sheet character){
    ...
    printf("Vida: %d\n", character.life);
    printf("Força: %d, Destreza: %d, Mana: %d\n", character.str, character.dex, character.mana);
...
void attack(Sheet* attacker, Sheet* defender) {
    ...
    int dano = attacker->str - (defender->dex / 2);
    if (dano < 0) dano = 0;
...
void openInventory(Sheet* character) {
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (item != NULL) {
            printf("Slot %d: %s (Poder: %d, Durabilidade: %d)\n", i, item->name, item->power, item->durability);
        }
    }
}